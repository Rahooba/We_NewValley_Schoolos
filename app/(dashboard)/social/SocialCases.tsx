...
import { createSocialCase, updateSocialCase, deleteSocialCase, type ActionState } from './actions';

export type SocialCaseItem = {
  id: string;
  studentId: string;
  studentName: string;
  title: string;
...
export function SocialCases({
  ...
  cases: SocialCaseItem[];
  students: StudentItem[];
}) {
  const [createOpen, setCreateOpen] = useState(false);
  const [edit, setEdit] = useState<SocialCaseItem | null>(null);
  const [createState, createAction] = useActionState(createSocialCase, initial);
  const [updateState, updateAction] = useActionState(updateSocialCase, initial);
        ...
        <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
          ...
          <thead className="bg-paper text-muted text-right">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">موعد المتابعة</th>
              <th className="px-4 py-2 font-medium">آخر تحديث</th>
              ...
              return (
                <tr key={c.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-2 text-muted">
                    {c.followUpAt ? new Date(c.followUpAt).toLocaleDateString('ar-EG') : '—'}
                  </td>
                  <td className="px-4 py-2 text-muted">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <FolderOpen size={14} /> تفاصيل / متابعة
                      </button>
                      <DeleteButton onDelete={() => deleteSocialCase(c.id)} />
                    </div>
                  </td>
                </tr>
              );
            ...
            {cases.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-muted">
                  لا توجد حالات اجتماعية مسجلة بعد
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="card w-full max-w-md p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display">تسجيل حالة اجتماعية</h3>
            ...
            <form action={createAction} className="space-y-3">
              ...
              <div>
                ...
                <input name="title" required className="input-field text-sm" placeholder="مثال: غياب متكرر" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs text-muted mb-1">موعد المتابعة</label>
                <input type="date" name="followUpAt" className="input-field text-sm" />
              </div>
              <CaseSubmit />
            ...
            <form action={updateAction} className="space-y-3">
              ...
              <div>
                ...
                <input
                  type="date"
                  name="followUpAt"
                  defaultValue={edit.followUpAt ? edit.followUpAt.slice(0, 10) : ''}
                  className="input-field text-sm"
                />
              </div>
              <div>
                <label className="block text-xs text-muted mb-1">ملاحظات المتابعة</label>
                <textarea
                  name="notes"
                  rows={4}
